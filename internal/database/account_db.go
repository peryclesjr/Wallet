...
func (a *AccountDB) Get(id string) (*entity.Account, error) {
	...
	err = row.Scan(&account.ID, &account.Client.ID, &account.Balance, &account.CreatedAt, &account.Client.ID, &account.Client.Name, &account.Client.Email, &account.Client.CreatedAt)
	if err != nil {
		return nil, err
	}
	return &account, nil
